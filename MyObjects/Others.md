...
代表的なものにロジスティック写像（値が極端な時に次の値が小さくなる、小さな初期値の変化が最終的に大きな変化をもたらす）がある  
　  
...
リアプリノフ指数:
各時点における微分係数（傾き）の絶対値の合計で定義  
正の場合、初期値の変化で将来の値が大きく変化すること意味し、カオス時系列と判定される  
サロゲートテスト:
データにノイズを加えた時のリアプリノフ指数の変化で定義  
